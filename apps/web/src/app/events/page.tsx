...
import { CalendarDays, ArrowRight, Users, Building2, RotateCcw } from 'lucide-react';
...
export default function EventsPage() {
  ...
  const monthDays = useMemo(() => {
    ...
    if (!Number.isFinite(year) || !Number.isFinite(month)) {
      return [] as Array<{ day: number | null; dateKey: string | null }>;
    }
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      const monthMatch = selectedMonth ? eventDateKey.startsWith(selectedMonth) : true;
      const dateMatch = selectedDate ? eventDateKey === selectedDate : true;
  ...
  const { upcomingEvents, pastEvents } = useMemo(() => {
    ...
    for (const event of filteredEvents) {
      if (new Date(event.eventDate).getTime() >= now) {
        upcoming.push(event);
      } else {
        past.push(event);
      }
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-background">
        <section className="relative overflow-hidden border-b bg-gradient-to-br from-primary/10 via-background to-emerald-400/10">
          ...
          <div className="container relative py-14 md:py-20">
            <div className="max-w-3xl space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight">Discover the latest events</h1>
              <p className="text-base md:text-lg text-muted-foreground">
                Explore upcoming and recent events from verified suppliers, with a smooth and responsive browsing experience.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="container py-10 md:py-14 space-y-10">
          {isLoading ? (
            ...
            <div className="rounded-2xl border p-8 text-center">
              <h2 className="text-2xl font-semibold">No events available yet</h2>
            ...
            <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedDate && !selectedDate.startsWith(e.target.value)) {
                          setSelectedDate('');
                        }
                      }}
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      {monthDays.map((cell, index) => {
                        if (!cell.day || !cell.dateKey) {
                          return <div key={`empty-${index}`} className="aspect-square rounded-md bg-muted/30" />;
                        }
                        ...
                        return (
                          <button
                            ...
                            className={`aspect-square rounded-md border text-[11px] flex flex-col items-center justify-center transition ${
                              count > 0 ? 'border-primary/50 bg-primary/10 text-primary' : 'border-border bg-background text-muted-foreground'
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">{upcomingEvents.length} found</span>
                </div>

                {upcomingEvents.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No upcoming events for now.</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                      ...
                      >
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          <p className="text-sm text-muted-foreground line-clamp-2">
                            {event.description || 'Event details are available on the event page.'}
                          </p>
                          ...
                          <div className="pt-1 flex items-center justify-between gap-2">
                            {event.supplier?.slug ? (
                              ...
                              <Link href={`/specialists/${event.specialist.id}`} className="text-xs text-primary hover:underline line-clamp-1">
                                {event.specialist.firstName || event.specialist.email || 'Specialist profile'}
                              </Link>
                            ) : <span />}

                            <Link href={`/events/${event._id}`} className={buttonVariants({ size: 'sm' })}>
              ...
              {pastEvents.length > 0 ? (
                <section className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold">Latest past events</h2>
                    <span className="text-sm text-muted-foreground">{pastEvents.length} found</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {pastEvents.slice(0, 6).map((event) => (
                      <Link
                        ...
                        className="rounded-xl border bg-card/70 p-4 transition-colors hover:bg-card"
                      >
                        <p className="text-sm font-medium line-clamp-1">{event.title}</p>
                        <p className="text-xs text-muted-foreground mt-1">
                          {new Date(event.eventDate).toLocaleDateString()} · {event.organizer}
                        </p>
                      </Link>
                    ))}
                  </div>
