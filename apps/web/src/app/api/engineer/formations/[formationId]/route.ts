...
export async function GET(_: NextRequest, { params }: { params: { formationId: string } }): Promise<NextResponse> {
  try {
    await connectDB();
    const session = await requireRole(...ALLOWED_ROLES);

    if (!mongoose.isValidObjectId(params.formationId)) {
      return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
    }

    const filter: Record<string, unknown> = { _id: params.formationId };
    if (session.role !== 'ADMIN') filter.specialistId = session.userId;

    const formation = await Formation.findOne(filter).lean();
    if (!formation) {
...
export async function PATCH(req: NextRequest, { params }: { params: { formationId: string } }): Promise<NextResponse> {
  try {
    await connectDB();
    const session = await requireRole(...ALLOWED_ROLES);
    ...
    const existing = await Formation.findOne(filter);
    if (!existing) {
      return NextResponse.json({ error: 'Formation introuvable' }, { status: 404 });
    }
    ...
    const formation = await Formation.findByIdAndUpdate(existing._id, { $set: updates }, { new: true }).lean();
...
export async function DELETE(_: NextRequest, { params }: { params: { formationId: string } }): Promise<NextResponse> {
  try {
    await connectDB();
    const session = await requireRole(...ALLOWED_ROLES);
    ...
    const existing = await Formation.findOne(filter).select('_id');
    if (!existing) {
      return NextResponse.json({ error: 'Formation introuvable' }, { status: 404 });
    }

    // Cascade-delete participations, then the formation itself.
    await FormationParticipation.deleteMany({ formationId: existing._id });
    await Formation.deleteOne({ _id: existing._id });
